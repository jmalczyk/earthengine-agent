...
async def call_geeviz_mcp(tool_name: str, arguments: dict) -> Any:
    """Calls a tool on the GeeViz MCP server.
...
async def visualize_data_with_geeviz(ee_object_expr: str, viz_params: dict, name: str) -> str:
    """Visualizes Earth Engine data using geeViz by executing code on the MCP server.

    Args:
        ee_object_expr (str): Python expression evaluating to an Earth Engine object (e.g., "ee.Image('USGS/SRTMGL1_003')").
        viz_params (dict): Visualization parameters.
        name (str): Layer name.

    Returns:
        str: A message indicating the result and the map URL.
    """
    code = f"""
import geeViz.geeView as gv
import ee
obj = {ee_object_expr}
gv.Map.addLayer(obj, {viz_params}, "{name}")
"""
...
async def visualize_change_with_geeviz(geometry: ee.Geometry | str) -> str:
    """Visualizes the magnitude of change using geeViz and returns a map URL.

    Args:
        geometry (ee.Geometry | str): An Earth Engine geometry object or a JSON string representing a GeoJSON geometry.

    Returns:
        str: A message with the URL to view the map.
    """
    if isinstance(geometry, str):
        geojson_str = geometry
    else:
        geojson_str = json.dumps(geometry.getInfo()) if hasattr(geometry, 'getInfo') else str(geometry)
...
def get_change_magnitude_image() -> ee.Image:
    ...
    def annual_angles(year: int) -> ee.Image:
        cur = embeddings.filter(ee.Filter.calendarRange(year, year, "year")).mosaic()
        prev_year = ee.Number(year).subtract(1)
        prev = embeddings.filter(ee.Filter.calendarRange(prev_year, prev_year, "year")).mosaic()
        return get_angle(prev, cur).rename(ee.Number(year).format("%d"))

    angles = ee.ImageCollection.fromImages(years.map(annual_angles)).toBands()
    return angles.reduce(ee.Reducer.max()).rename("magnitude")
...
gv.Map.centerObject(region)
gv.Map.addLayer(magnitude_image, viz_params, "Change Magnitude")
"""
    await call_geeviz_mcp("run_code", {"code": code})
    url = await call_geeviz_mcp("map_control", {"action": "view"})
